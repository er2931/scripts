...
# Pitch depends ONLY on Tone and y_amp
# -------------------------------------------------
...
class AudioEngine:
    ...
    def __init__(self, func_provider):
        ...
        self.current_y_at_dot = 0.0  # actually y_amp now
    ...
    def _callback(self, outdata, frames, time_info, status):
        ...
        # ---- y_amp: amplitude-scaled y, using audio phase (no visual freq/speed) ----
        y_raw = func(self.phase)
        ...
        amp_norm = max(0.0, min(1.0, abs(y_amp)))
        dyn = 0.5 + amp_norm  # 0.5..1.5, tune this if you want subtler/stronger pitch swing
        ...
        for i in range(frames):
            ...
            # Audio time: independent of visual frequency & speed
            t = self.phase
...
"""
    ...
    def update_y_display(self):
        ...
        # For display, tie y_raw to audio phase if audio is active,
        # so y_amp shown corresponds to what controls pitch.
        if self.audio_engine is not None:
            t_center = self.audio_engine.phase
        else:
            t_center = math.pi
