...
SAFE_MATH_NAMESPACE: Dict[str, object] = {
    ...
    "ceil": math.ceil,
    "abs": abs,
    "math": math,  # allow expressions that explicitly use math.sin, math.fabs, etc.
}
...
# -------------------------------------------------
# DEFAULT FUNCTIONS (fallback) + custom patterns
# -------------------------------------------------
...
def get_default_functions() -> List[FunctionDef]:
    return [
        ...
        # ---- Your custom rectification & pattern shapes ----

        FunctionDef(
            name="lro",
            label="Limit Rectification (LRO)",
            expr_str="(t/0.6 if t < 0.6 else 1.0) + 0.06*math.sin(10*math.pi*t)",
            func=make_func_from_expr(
                "(t/0.6 if t < 0.6 else 1.0) + 0.06*math.sin(10*math.pi*t)"
            ),
            color="#40e9ff",
        ),
        FunctionDef(
            ...
            expr_str=(
                "0.0 if (t < 0.2 or t > 0.8) else "
                "(lambda u: math.sin(math.pi*u) + 0.35*math.sin(2*math.pi*u))((t-0.2)/0.6)"
            ),
            func=make_func_from_expr(
                "0.0 if (t < 0.2 or t > 0.8) else "
                "(lambda u: math.sin(math.pi*u) + 0.35*math.sin(2*math.pi*u))((t-0.2)/0.6)"
        ...
        FunctionDef(
            ...
            expr_str=(
                "(t/0.2 if t < 0.2 else "
                "(1.0 if t < 0.8 else "
                "((1.0 - (t-0.8)/0.2) if (1.0 - (t-0.8)/0.2) > 0.0 else 0.0)))"
            ),
            func=make_func_from_expr(
                "(t/0.2 if t < 0.2 else "
                "(1.0 if t < 0.8 else "
        ...
        FunctionDef(
            ...
            expr_str="0.5*(1 - math.cos(2*math.pi*t))*(0.5 + 0.4*math.sin(16*math.pi*t))",
            func=make_func_from_expr(
                "0.5*(1 - math.cos(2*math.pi*t))*(0.5 + 0.4*math.sin(16*math.pi*t))"
            ),
            color="#25f0c1",
        ),
        FunctionDef(
            ...
            label="Hollow Void",
            expr_str=(
                "(0.6*math.sin(4*math.pi*t) if not (0.4 < t and t < 0.6) else "
                "(0.6*math.sin(4*math.pi*t)*("
                "(1.0 - math.fabs(t-0.5)/0.1) if (1.0 - math.fabs(t-0.5)/0.1) > 0.0 else 0.0)))"
            ),
            func=make_func_from_expr(
                "(0.6*math.sin(4*math.pi*t) if not (0.4 < t and t < 0.6) else "
                "(0.6*math.sin(4*math.pi*t)*("
        ...
        FunctionDef(
            ...
            label="Fractal Echo",
            expr_str=(
                "((1.0*math.sin(2*math.pi*1*t + 1*0.4) + "
                "(1.0/2.0)*math.sin(2*math.pi*2*t + 2*0.4) + "
                "(1.0/3.0)*math.sin(2*math.pi*3*t + 3*0.4) + "
                "(1.0/4.0)*math.sin(2*math.pi*4*t + 4*0.4))/2.5)"
            ),
            func=make_func_from_expr(
                "((1.0*math.sin(2*math.pi*1*t + 1*0.4) + "
                "(1.0/2.0)*math.sin(2*math.pi*2*t + 2*0.4) + "
        ...
        FunctionDef(
            name="photon_veins",
            label="Photon Veins",
            expr_str="0.4*math.sin(2*math.pi*t) - 1.2*math.exp(-((math.fabs(t-0.5)*40.0)**2))",
            func=make_func_from_expr(
                "0.4*math.sin(2*math.pi*t) - 1.2*math.exp(-((math.fabs(t-0.5)*40.0)**2))"
            ),
            color="#ff944d",
        ),
        FunctionDef(
            ...
            expr_str="0.7*math.sin(2*math.pi*t) + 0.4*math.sin(4*math.pi*t + 0.7) + 0.25*math.sin(14*math.pi*t + 0.3)",
            func=make_func_from_expr(
                "0.7*math.sin(2*math.pi*t) + 0.4*math.sin(4*math.pi*t + 0.7) + 0.25*math.sin(14*math.pi*t + 0.3)"
            ),
            color="#ff5cf0",
        ),
        FunctionDef(
            ...
            expr_str=(
                "((0.0 if (t < 0.15 or t > 0.85) else "
                "(1.4*math.exp(-((t-0.35)*40.0)**2))) + 0.25*math.sin(2*math.pi*2*t))"
            ),
            func=make_func_from_expr(
                "((0.0 if (t < 0.15 or t > 0.85) else "
                "(1.4*math.exp(-((t-0.35)*40.0)**2))) + 0.25*math.sin(2*math.pi*2*t))"
        ...
        FunctionDef(
            ...
            expr_str="0.4*math.sin(2*math.pi*t) + 0.3*math.sin(6*math.pi*t + 0.5) + 0.2*math.sin(10*math.pi*t + 1.1)",
            func=make_func_from_expr(
                "0.4*math.sin(2*math.pi*t) + 0.3*math.sin(6*math.pi*t + 0.5) + 0.2*math.sin(10*math.pi*t + 1.1)"
            ),
            color="#ffcf40",
        ),
        FunctionDef(
            ...
            expr_str="0.6*math.sin(2*math.pi*(3*t*t + 0.2)) + 0.3*math.sin(2*math.pi*(9*t + 0.4))",
            func=make_func_from_expr(
                "0.6*math.sin(2*math.pi*(3*t*t + 0.2)) + 0.3*math.sin(2*math.pi*(9*t + 0.4))"
            ),
            color="#7d7dff",
        ),
    ]
...
# Pitch depends ONLY on Tone and y_amp (center dot), signed
# -------------------------------------------------
...
class AudioEngine:
    ...
    def set_tone(self, t: float):
        # 0..1 slider -> base pitch position in range
        self.tone = max(0.0, min(1.0, t))

    def _compute_pitch_from_y(self, y_amp: float) -> float:
        """
        Pitch depends ONLY on:
          - Tone slider (base pitch in [pitch_min_hz, pitch_max_hz])
          - Signed y_amp of the center dot

        Behavior:
          * y_amp > 0 -> pitch ABOVE base
          * y_amp < 0 -> pitch BELOW base
          * y_amp ~ 0 -> near base
        """
        # Clamp tone-controlled base pitch
        span = max(1.0, self.pitch_max_hz - self.pitch_min_hz)
        base_freq = self.pitch_min_hz + self.tone * span

        # y_amp is typically around [-1.5, 1.5], clamp to [-1, 1]
        y_norm = max(-1.0, min(1.0, y_amp))

        # Depth of modulation:
        #   y_norm = +1 => base * (1 + depth)
        #   y_norm = -1 => base * (1 - depth)
        depth = 0.9  # 90% up or down from base at extremes
        dyn = 1.0 + depth * y_norm  # in [0.1, 1.9]

        if dyn < 0.1:
            dyn = 0.1

        freq_hz = base_freq * dyn
        freq_hz = max(self.pitch_min_hz, min(self.pitch_max_hz, freq_hz))
        return freq_hz
    ...
    def _callback(self, outdata, frames, time_info, status):
        ...
        # Clamp amplitude safely (visual amplitude)
        amplitude = max(0.0, min(1.5, amplitude))

        # ---- y_amp AT THE SAME DOT AS THE CANVAS ----
        # Canvas center dot uses:
        #   base_center = pi
        ...
        self.current_y_at_dot = y_amp  # for debug / GUI

        # ---- PITCH: ONLY Tone (base) + y_amp (signed) ----
        freq_hz = self._compute_pitch_from_y(y_amp)
        ...
        for i in range(frames):
            ...
            # Audio oscillator time – free-running phase for the SHAPE only
            t = self.phase
...
class MainWindow(QMainWindow):
    ...
    def _apply_panel_theme(self, panel: QFrame):
        # Dark neon UI – buttons & sliders made cooler
        panel.setStyleSheet(
            f"""
QFrame {{
...
QLabel, QLineEdit, QPlainTextEdit, QListWidget, QComboBox {{
  font-size: 9pt;
  color: {self.theme.panel_text};
...
QComboBox::drop-down {{
  width: 18px;
  border-left: 1px solid #30324A;
  background-color: #1E2035;
}}
QComboBox QAbstractItemView {{
  background-color: #141627;
  border: 1px solid #30324A;
  selection-background-color: #30335A;
}}
