...
#   - Shows that member's Discord avatar on the RIGHT side of the window
...
class StatusBitApp(QWidget):
    ...
    def _build_ui(self):
        ...
        # Poll + start/stop (settings button moved to bottom)
        row_box = QHBoxLayout()
        ...
        # Settings button at bottom-right of LEFT column
        ...
        settings_bar.addStretch(1)                 # push button to the right
        settings_bar.addWidget(self.settings_btn)  # bottom-right of window
        right.addLayout(settings_bar)


        # Add left and right to root
        # Add left and right to root, force LEFT panel to the left edge
        # Add left and right to root
        root.addLayout(left, 4)
        root.addLayout(right, 1)

        # (Optional) if you really want to force left column alignment:
        # root.setAlignment(left, Qt.AlignmentFlag.AlignLeft)
