...
def save_config(cfg):
    # convert Alarm objects to dicts
...
class FloatingClock(QWidget):
    ...
    def fire_alarm(self, alarm: Alarm):
        # Simple: show small message box & open file / sound
        label = alarm.label or "Alarm"
        try:
            # Don’t block the clock forever; use a non-modal message
        ...
        if alarm.path:
            # Both actions use os.startfile; type is just semantic
            if hasattr(os, "startfile"):
                try:
                    os.startfile(alarm.path)
                except Exception:
                    pass
...
class AlarmDialog(QDialog):
    def __init__(self, parent=None, alarm: Alarm | None = None):
        ...
        self.path_edit = QLineEdit(self.alarm.path)
    ...
    def get_alarm(self) -> Alarm | None:
        ...
        # minimal validation: expect 5 chars "HH:mm"
...
class SettingsWindow(QWidget):
    ...
    def refresh_alarm_list(self):
        ...
        for alarm in self.config.get("alarms", []):
            ...
            text = f"[{status}] {alarm.time}  –  {alarm.label}  ({action_text})"
